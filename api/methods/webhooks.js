...
const { oneSunday } = require("../services/airtable");

const square = async (event, context, callback) => {
  try {
    let result = {};
    ...
    switch (event_type) {
      case "PAYMENT_UPDATED":
        ...
        let itemizations = await Promise.map(tx.itemizations, async item => {
        ...
        await Promise.map(itemizations, item =>
        ...
        await oneSunday("Reports").update(currentQuarter.id, {
          ...
          "Sales Tax Collected": currentQuarter.get("Sales Tax Collected") + (tx.tax_money.amount / 100),
          "CC Processing Fees": currentQuarter.get("CC Processing Fees") + (-tx.processing_fee_money.amount / 100),
          "Refunds": currentQuarter.get("Refunds") + (-tx.refunded_money.amount / 100)
        });
        break;
    }
